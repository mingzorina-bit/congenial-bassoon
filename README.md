# BilingualInput — Candidate Foundation v0.0.1

Windows 原生输入原型，采用 WinUI 3/C# 与 C++20 Core、真实 librime。当前交付首个里程碑，等待实际试用反馈；完整 Phase 0 尚未完成。

## 试用
按 [体验指南](README-TRY.md) 下载成功 Actions 构建中的 BilingualInput-v0.0.1-win-x64，完整解压后运行 BilingualInput.exe。无需 API Key。有限自有词表用于验证候选交互；Shadow、学习、AI 与系统级输入尚未实现。
...
4. [本阶段执行证据](docs/releases/v0.0.1.md)。
...
成品位于 artifacts/BilingualInput-v0.0.1-win-x64.zip。GitHub Actions 使用同一流程。包内附来源、许可和文件校验清单。不要只拷贝 exe；原型不安装为系统输入法。
